...
//*	 ENABLE
	GetCtrlVal(D_panelHandle, DPANEL_pixel_nm, &pixel_nm);
	GetCtrlVal(D_panelHandle, DPANEL_conversion_pix_nm, &conversion_pix_nm);
	...
	if(pixel_nm==0)
			...
			else
			{
			 ...
			 for(int a=0; a<2068;a++)
			  {
			  Xarray[b*2068+a]= (a+1)*conversion_pix_nm;
			  }
			  pixel_nm=0;
			 }
			}
...
int select_path()
{
   int ipath;
	...
	ipath = DirSelectPopup ("d:\\data", "Select output directory", 0, 1, pathname);
	SetCtrlVal(D_panelHandle, DPANEL_actual_dir, pathname);
	global_file_counter=0;
	
return ipath; // if ipath = 0 re-ask if necessary
}

void save_spectra(void)
{
...
sprintf(dummy, "ss%i.shots", global_file_counter);
...
void plot_spectra(void)
{
    ...
    PlotXY (D_panelHandle, DPANEL_ccd1, Xarray[i*2068], array[i*2068],
        bytes, VAL_INTEGER, VAL_DOUBLE, VAL_THIN_LINE, VAL_EMPTY_SQUARE,
        VAL_SOLID, 1, VAL_RED);
